...
Object.values(passwordList).map((el) => {
    const constraints = el['times'].split('-').map((c) => parseInt(c));
    const occurence = Array.from(el['password']).reduce((pr, cv) => (cv == el['letter'] ? 1 : 0) + pr, 0);
    if(occurence >= constraints[0] && occurence <= constraints[1]) return 'VALID';
    else return 'INVALID';
});